...
def generate_unittest_task_from_redis(task_id):
    ...
    try:
        ...
        task_service.change_status_stat(task, TASK_RUN)
        ...
        processed_key = task_service.store_task_result(task, result_json)

        task_service.change_status_stat(task, TASK_SUCC)
        ...
        task_service.change_status_stat(task, TASK_FAIL)
        logging.error(f"Task {task_id} failed at {datetime.now()}: {str(e)}")
        return {'status': 'failed', 'error': str(e)}